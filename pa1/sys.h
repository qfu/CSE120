...
void RestoreContext (int p, CONTEXT *c);	// restore context of p from c
int SwitchContext (int);	// working version of switch context
int GetCurProc ();		// returns current process's id


/*	Scheduling
 */
void SetTimer (int);		// timer to interrupt after t ticks
int GetTimer ();		// returns the current value of timer
int Block (int);		// make proc p ineligible for sched
int Unblock (int);		// make proc p eligible for sched
int SetSchedPolicy (int);	// set the scheduling policy to p
int GetSchedPolicy ();		// return the scheduling policy
void DoSched ();		// make sched decision at next opportune time

/*	Other
 */
void Panic (char *);		// causes abnormal exit from kernel
